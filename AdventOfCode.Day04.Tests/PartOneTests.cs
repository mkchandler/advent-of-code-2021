...
public class PartOneTests
{
    ...
    [Fact]
    public void ReturnsBoardThatIsFirstToWin()
    {
        (IEnumerable<int> numbers, IList<Board> boards) = new Input().ParseInput(_input);
        var (lastNumberCalled, winningBoard) = new PartOne().PlayBingo(numbers, boards);
        int? losingNumbersSum = winningBoard?.Squares.Where(s => !s.Drawn).Sum(s => s.Value);

        Assert.Equal(24, lastNumberCalled);
        Assert.Equal(188, losingNumbersSum);
        Assert.Equal(4512, losingNumbersSum * lastNumberCalled);
    }
}